...
const STORAGE_KEY = 'fanpage_creation_state_v1'

function storageOf(storage) {
  return storage ?? (typeof globalThis !== 'undefined' ? globalThis.localStorage : null)
}

export function loadFanPageCreation(storage = null) {
...
export function saveFanPageCreation(record, storage = null) {
  const store = storageOf(storage)
...
export function clearFanPageCreation(storage = null) {
  const store = storageOf(storage)
  if (store) store.removeItem(STORAGE_KEY)
}

// 作成要求。既存の進行中レコードがあれば新しく作らず、それを返す。
export function beginFanPageCreation({ pageName, publicAddress, operationId, storage = null } = {}) {
  const existing = loadFanPageCreation(storage)
  ...
  return saveFanPageCreation(record, storage)
}

// 再開。保存済みの工程状態をそのまま渡すため、完了済みの工程は実行されない。
export async function runFanPageCreation({ record, adapter, storage = null, onEvent = () => {} } = {}) {
  ...
  const provisioning = await resumeProvisioning({
    ...
    onEvent: (event) => {
      ...
      saveFanPageCreation({ ...record, provisioning: { ...record.provisioning } }, storage)
      onEvent(event)
    },
  })
  return saveFanPageCreation({ ...record, provisioning }, storage)
}

// acquisition側が読む形へ変換する。
export function toFanPageStatus(record) {
...
// 顧客向けの表示。内部工程名は出さない。
export function describeFanPageCreation(record) {
  const status = record?.provisioning?.status
  if (!record) {
    return { headline: 'まだ歌推しページを作っていません', detail: '', tone: 'action_required', canRetry: false }
  }
  if (status === 'complete') {
    return {
  ...
  return {
    ...
    detail: '歌推しページの準備がまだ完了していません。完了すると次の設定へ進めます。',
    tone: 'waiting',
    canRetry: false,
  }
}
