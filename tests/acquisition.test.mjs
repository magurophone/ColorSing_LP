...
function fanPageStepOf(acquisition) {
  const model = deriveOnboardingSteps({ config: {}, acquisition })
  return model.steps.find(step => step.id === 'fanpage_created')
}
...
test('購入前から公開までの状態を、決済も認証も決めずに判定できる', () => {
  ...
  assert.equal(deriveAcquisitionState({ account: { status: 'ready' }, portal: { status: 'provisioning' } }), A.FANPAGE_PROVISIONING)
  assert.equal(deriveAcquisitionState({ portal: { status: 'ready' } }), A.FANPAGE_READY)
...
test('各状態に対応する画面が決まっている', () => {
  ...
  assert.equal(ACQUISITION_ROUTES[A.FANPAGE_NOT_CREATED], '/fanpage/create')
  assert.equal(ACQUISITION_ROUTES[A.FANPAGE_READY], '/onboarding')
})
...
test('providerは注入しない限り未設定を返し、決まったふりで進めない', () => {
  ...
  assert.equal(createProvisioningProvider({ readFanPage: async () => ({ status: 'ready' }) }).configured, true)
})

test('進めない理由を、操作待ち・処理待ち・失敗に分ける', () => {
  assert.equal(classifyBlocking(A.FANPAGE_NOT_CREATED), 'action_required')
  assert.equal(classifyBlocking(A.FANPAGE_PROVISIONING), 'waiting')
...
test('準備中は待ちであってエラーではない', () => {
  const guidance = describeFanPageStep(A.FANPAGE_PROVISIONING, { status: 'provisioning' })
  assert.equal(guidance.headline, '歌推しページを準備しています')
  assert.equal(guidance.action, null)
  assert.equal(guidance.blocking, 'waiting')
  assert.equal(fanPageStepOf({ account: { status: 'ready' }, portal: { status: 'provisioning' } }).status, S.PENDING)
})

test('実際に失敗したときだけBLOCKEDにし、やり直しを示す', () => {
  const step = fanPageStepOf({ account: { status: 'ready' }, portal: { status: 'failed' } })
  assert.equal(step.status, S.BLOCKED)
  assert.equal(step.guidance.action.label, 'もう一度試す')
})

test('利用者向け案内にシステム語彙を残さない', () => {
  const states = [A.VISITOR, A.ENTITLEMENT_GRANTED, A.FANPAGE_NOT_CREATED, A.FANPAGE_PROVISIONING]
  for (const state of states) {
    const guidance = describeFanPageStep(state)
...
test('獲得導線を渡さない従来の呼び出しは挙動を変えない', () => {
  const step = deriveOnboardingSteps({ config: {} }).steps.find(item => item.id === 'fanpage_created')
  assert.equal(step.status, S.BLOCKED)
